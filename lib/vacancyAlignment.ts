...
export function analyzeVacancyAlignment(job:JobLead,doc:CareerDocument):AlignmentReport{
 ...
 const knownParts=parts.filter(part=>(part.key!=="skills"||required.length>0)&&(part.key!=="responsibilities"||Boolean(description)));
 const recoverable=knownParts.reduce((n,p)=>n+(p.max-p.score),0),potential=Math.min(100,score+Math.round(recoverable*.65));
