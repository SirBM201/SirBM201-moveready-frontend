...
export default function PartnerApplicationForm() {
  ...
  const [message, setMessage] = useState("Submit provider details for review. Approved services can later be connected to routes, requests, and partner handoffs.");
  ...
  async function submitApplication(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setState("success");
      setMessage("Provider application submitted. MoveReady will review the service before any public listing or handoff.");
      form.reset();
    } catch (error) {
      const apiError = error as ApiError;
      setState("error");
      setMessage(apiError?.status === 503 ? "Unable to save application. Run SQL 011 and redeploy the backend." : "Unable to submit provider application.");
    }
  }
  ...
  return (
    <form className="interest-form" onSubmit={submitApplication}>
      <div>
        ...
        <p>Submit enough detail for screening. Public service access stays approval-based.</p>
      </div>
      ...
      <div className="field">
        ...
        <textarea id="credentials_summary" name="credentials_summary" rows={4} placeholder="Licences, courier relationship, insurance authorization, notary status, experience, or relevant proof." />
      </div>
      <div className="field">
        <label htmlFor="compliance_notes">Compliance and trust notes</label>
        <textarea id="compliance_notes" name="compliance_notes" rows={3} placeholder="How sensitive documents, refunds, privacy, tracking, and customer complaints are handled." />
      </div>
      <div className="field">
        <label htmlFor="pricing_notes">Pricing notes</label>
        <textarea id="pricing_notes" name="pricing_notes" rows={3} placeholder="Typical fees, commission model, discount options, or quote requirements." />
      </div>
