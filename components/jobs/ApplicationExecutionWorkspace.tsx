...
const label=(value?:string|null)=>(value||"not started").replace(/_/g," ").replace(/\b\w/g,c=>c.toUpperCase());
...
export default function ApplicationExecutionWorkspace(){
 ...
 const refreshGlobal=useCallback(async()=>{
  const[p,f,l]=await Promise.all([executionClient.portfolio.overview(),executionClient.followups.due(),executionClient.lifecycles.list()]);
  setPortfolio(p.items||[]);setPortfolioSummary(p.summary||null);setDue(f);setLifecycles(l);
 },[]);
 const load=useCallback(async()=>{
  ...
  try{
   ...
   const first=(requested&&rows.some(row=>row.id===requested)?requested:"")||selectedId||rows[0]?.id||"";setSelectedId(first);if(first)await loadJob(first);
 ...
 const topCommand=portfolio.find(item=>item.next_action?.type&&item.next_action.type!=="none");
 ...
 return <main className={styles.shell}>
  <section className={styles.hero}><div><p className="overline">LQ16 · guided application execution</p><h1>Turn readiness gaps into one controlled next action.</h1><p>Move from qualification evidence to application preparation, explicit approval, manual employer handoff and evidence-backed follow-up. MoveReady never submits or contacts an employer for you.</p></div><a className="btn" href="/jobs/applications">Open legacy tracker</a></section>
  <section className={styles.summary} aria-label="Portfolio summary">
   <div className={styles.metric}><strong>{portfolioSummary?.actionable??portfolio.length}</strong><span>Actions ready</span></div><div className={styles.metric}><strong>{portfolioSummary?.blocking??0}</strong><span>Blocking gaps</span></div><div className={styles.metric}><strong>{portfolioSummary?.ready_to_apply??0}</strong><span>Ready to apply</span></div><div className={styles.metric}><strong>{portfolioSummary?.due_followups??due.length}</strong><span>Follow-ups due</span></div>
  </section>
  {topCommand?<section className={styles.panel} aria-label="Highest priority application command"><div><p className="overline">Do this next</p><h2>{topCommand.next_action?.title||label(topCommand.next_action?.type)}</h2><p><strong>{topCommand.job_title||"Recorded vacancy"}</strong> at {topCommand.company_name||"the recorded employer"} · Priority {topCommand.priority_score??0}</p></div><a className="btn primary" href={topCommand.next_action?.href||`/jobs/execution?jobId=${encodeURIComponent(topCommand.job_id)}`}>Open next action</a></section>:null}
  ...
  <div className={styles.layout}>
   ...
   <section className={styles.pipeline} aria-label="Controlled application pipeline">
    <article className={styles.step}><div className={styles.stepHead}><div><p className="overline">1 · readiness</p><h2>Bind active documents</h2></div><span className={`${styles.state} ${readiness?.readiness?.blocking_issue_count?styles.blocked:styles.attention}`}>{label(state)}</span></div>
     ...
     {readiness?.readiness?.issues?.length?<ul className={styles.issues}>{readiness.readiness.issues.map(issue=><li key={issue.code}><strong>{label(issue.severity||issue.category)}</strong> · {issue.message}{issue.blocking?" — blocking":""}</li>)}</ul>:<p>No current readiness blockers are reported.</p>}
